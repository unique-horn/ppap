...
class PPNGen(object):
    ...
    def _generator(self):
        ...
        coordinates
        """
        coordinates_values = self.generate_coordinates()
        coordinates = K.variable(value=coordinates_values, name="coordinates")

        layer_outs = [K.tanh(T.dot(coordinates, self.weights[0]))]

        for i in range(1, len(self.weights)):
            layer_outs.append(K.tanh(T.dot(layer_outs[i - 1], self.weights[i])))

        rows, cols = self.output_shape

        final_filter = layer_outs[-1].reshape((1, 1, rows, cols))

        return [final_filter, self.weights]
    ...
    def generate_coordinates(self):
        ...
        return np.vstack([X_r, Y_r, R_r]).transpose()